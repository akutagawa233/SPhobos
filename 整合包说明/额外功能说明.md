...
```ini
[SOMEUNIT]                                   ; 只能是车辆
Locomotor=Skilled                            ; 移动方式，以下功能仅在此移动方式下可用
Skilled.ReverseSpeed=0.85                    ; 浮点型，倒车时的速度倍率
Skilled.FaceTargetRange=16.0                 ; 浮点型，当单位指定了目标时，离目标距离多远内，会倒车来保持正面相对
Skilled.RetreatDuration=150                  ; 整数型，当单位制定了目的地时，距上次受到伤害这么长时间内，会倒车前往目的地
```

```{note}
- 一般情况下，都会正面朝向移动方向
- 所有使用限制和 `Locomotor=Drive` 相同
- 同时，`Locomotor=Skilled` 相比 `Locomotor=Drive` 修复了在进行日字形移动时不会碾压到中间格的问题，且上下坡转动将更为顺滑
```
